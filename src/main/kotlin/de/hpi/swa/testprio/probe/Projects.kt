...
object Projects {
    ...
    fun firstFailuresOf(context: DSLContext, projectName: String): List<String> =
            context.select(field(name("tt1", "tr_job_id")))
                .from(
                    table(name("travistorrent_8_2_2017")).`as`("tt1"),
                    table(name("travistorrent_8_2_2017")).`as`("tt2")
                ).where(
                    field(name("tt1", "gh_project_name")).eq(projectName)
                    .and(field(name("tt2", "gh_project_name")).eq(projectName))
                    .and(field(name("tt1", "tr_build_number")).minus(1).eq("tt2.tr_build_number"))
                    .andExists(
